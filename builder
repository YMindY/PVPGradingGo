...
namespace{
	...
	abstract class Package{
		...
		public function __construct($BuildTag){
			...
			*/
			self::$PharPath=self::$Build.'DHDJ-PVPGradingGo-'.date("Ymdhis").'-'.self::$BuildTag.'-Build-'.self::$BuildNum.'.phar';
		...
		public function packPhar(){
			...
			$this->Phar->buildFromIterator(new RecursiveIteratorIterator(new RecursiveDirectoryIterator(self::$Base.'src/org/hypergo/PVPGradingGo/')),self::$Base.'src/org/hypergo/PVPGradingGo/');
			$this->syncTimer();
			$this->Phar->compressFiles(Phar::GZ);
			$this->syncTimer();
			$this->Phar->stopBuffering();
			$this->syncTimer();
			$this->Phar->setStub('<?php if(file_exists("phar://" . __FILE__ . "/src/pocketmine/PocketMine.php")){require("phar://" . __FILE__ . "/src/pocketmine/PocketMine.php");} else {echo "PVPGradingGo needs PocketMine environment.";}__HALT_COMPILER();');
